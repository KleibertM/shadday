...
if (!empty($_POST['additem'])) {
    if (empty($_POST['nombre'])|| empty($_POST['foto']) || empty($_POST['version']) || empty($_POST['categoria']) || empty($_POST['editorial']) || empty($_POST['fecha']) || empty($_POST['stock']) || empty($_POST['precio'])) {
        ...
        $stmt = $conexion->prepare("INSERT INTO item (nombre, foto, version, categoria, editorial, fecha, stock, precio) VALUES ( :nombre, :foto, :version, :categoria, :editorial, :fecha, :stock, :precio)");
